...
const CSS = `
  @keyframes bit-chat-fade-in { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
  @keyframes bit-chat-dot { 0%, 80%, 100% { opacity: 0.25; } 40% { opacity: 1; } }
  ...
  .bit-chat-bubble-btn:hover { transform: scale(1.06); }
  ...
  .bit-chat-panel {
    ...
    animation: bit-chat-fade-in 0.18s ease;
  }

  .bit-chat-msg-row { display: flex; margin-bottom: 10px; }
  .bit-chat-msg-row.user { justify-content: flex-end; }
  .bit-chat-bubble {
    max-width: 80%; padding: 9px 13px; border-radius: 12px;
    font-size: 13px; line-height: 1.55; white-space: pre-wrap; word-break: break-word;
  }
  .bit-chat-bubble.assistant { background: #F9F7F1; border: 1px solid #E8E4D9; color: #0F1F3D; border-bottom-left-radius: 3px; }
  .bit-chat-bubble.user { background: #0F1F3D; color: #fff; border-bottom-right-radius: 3px; }
  ...
  .bit-chat-input:focus { border-color: #C9A84C; background: #fff; box-shadow: 0 0 0 3px rgba(201,168,76,0.14); }
  .bit-chat-input:disabled { opacity: 0.6; cursor: not-allowed; }
  ...
  .bit-chat-send-btn:disabled { opacity: 0.4; cursor: not-allowed; }

  .bit-chat-dot { display: inline-block; width: 5px; height: 5px; border-radius: 50%; background: #8A8070; margin-right: 3px; animation: bit-chat-dot 1.2s infinite; }
  .bit-chat-dot:nth-child(2) { animation-delay: 0.15s; }
  .bit-chat-dot:nth-child(3) { animation-delay: 0.3s; }
`

export function MenteeChatWidget() {
  ...
  async function loadHistory() {
    ...
    try {
      const res = await api.get('/mentee/me/chat/history')
  ...
  async function handleSend() {
    ...
    try {
      const res = await api.post('/mentee/me/chat', { message: trimmed })
          ...
          <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '16px 14px' }}>
            {loadingHistory ? (
              <div style={{ textAlign: 'center', padding: '24px 0' }}>
                <i className="ti ti-loader-2" style={{ fontSize: 20, color: '#C9A84C', animation: 'bit-chat-dot 1s linear infinite' }} />
              </div>
            ) : messages.length === 0 ? (
              <div className="bit-chat-msg-row assistant">
                <div className="bit-chat-bubble assistant">
                  Hi! I'm your Build In Tech Assistant. Ask me anything about your track, the program, or if you're stuck on a concept. I won't do your assignments for you, but I'll help you understand them.
                </div>
              </div>
            ) : (
              messages.map((m, i) => (
                <div key={m.id || i} className={`bit-chat-msg-row ${m.role}`}>
                  <div className={`bit-chat-bubble ${m.role}`}>{m.content}</div>
                </div>
              ))
            )}
            ...
            {sending && (
              <div className="bit-chat-msg-row assistant">
                <div className="bit-chat-bubble assistant">
                  <span className="bit-chat-dot" /><span className="bit-chat-dot" /><span className="bit-chat-dot" />
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ borderTop: '1px solid #E8E4D9', padding: '10px 14px', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
              <textarea
                ...
                rows={1}
                className="bit-chat-input"
              />
              <button
                onClick={handleSend}
                disabled={sending || limitReached || !input.trim()}
                className="bit-chat-send-btn"
